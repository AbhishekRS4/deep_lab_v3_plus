...
class DeepLab3Plus:
    def __init__(self, pretrained_weights, is_training, data_format='channels_first', num_classes=15):
        ...
        self._encoder_dilations = [1, 1, 2, 2]
    ...
    def resnet50_encoder(self, features):
        ...
        # Stage 1
        self.stage1 = tf.nn.max_pool(self.stage0, ksize=self._encoder_pool_kernel, strides=self._encoder_pool_strides,
        ...
        self.stage2 = self._res_identity_block(
            input_layer=self.stage2, stage='2b')
        self.stage2 = self._res_identity_block(
            input_layer=self.stage2, stage='2c')
        ...
        self.stage3 = self._res_identity_block(
            input_layer=self.stage3, stage='3b')
        self.stage3 = self._res_identity_block(
            input_layer=self.stage3, stage='3c')
        self.stage3 = self._res_identity_block(
            input_layer=self.stage3, stage='3d')
        ...
        self.stage4 = self._res_identity_block(
            input_layer=self.stage4, stage='4b')
        self.stage4 = self._res_identity_block(
            input_layer=self.stage4, stage='4c')
        self.stage4 = self._res_identity_block(
            input_layer=self.stage4, stage='4d')
        self.stage4 = self._res_identity_block(
            input_layer=self.stage4, stage='4e')
        self.stage4 = self._res_identity_block(
            input_layer=self.stage4, stage='4f')

        # Stage 5
        self.stage5 = self._res_conv_block(
            input_layer=self.stage4, stage='5a', strides=self._encoder_conv_strides, dilations=self._encoder_dilations)
        self.stage5 = self._res_identity_block(
            input_layer=self.stage5, stage='5b', dilations=self._encoder_dilations)
        self.stage5 = self._res_identity_block(
            input_layer=self.stage5, stage='5c', dilations=self._encoder_dilations)

    # build deep_lab_v3+
    def deeplabv3_plus(self):
        self.aspp_out = self._atrous_spatial_pyramid_pool_block(
            self.stage5, name='aspp_')
        ...
        self.up1_concat = tf.concat(
            [self.up1, self.low_level_features], axis=self._feature_map_axis, name='decoder_concat')

        self.decoder_conv1 = self._get_conv2d_layer(
            self.up1_concat, 256, [3, 3], [1, 1], name='decoder_conv1')
        self.decoder_conv1 = self._get_relu_activation(
            self.decoder_conv1, name='decoder_relu1')
        self.decoder_conv2 = self._get_conv2d_layer(
            self.decoder_conv1, 256, [3, 3], [1, 1], name='decoder_conv2')
        self.decoder_conv2 = self._get_relu_activation(
            self.decoder_conv2, name='decoder_relu2')
    ...
    def _atrous_spatial_pyramid_pool_block(self, input_layer, depth=256, name='aspp_'):
        ...
        _conv1x1 = self._get_conv2d_layer(input_layer, depth, [1, 1], [
                                          1, 1], name=name + 'conv1x1')
        _conv1x1 = self._get_relu_activation(
            _conv1x1, name=name + 'conv1x1_relu')

        _conv3x3_1 = self._get_conv2d_layer(input_layer, depth, [3, 3], [
                                            1, 1], dilation_rate=self._atrous_rate[0], name=name + 'conv3x3_1')
        _conv3x3_1 = self._get_relu_activation(
            _conv3x3_1, name=name + 'conv3x3_1_relu')

        _conv3x3_2 = self._get_conv2d_layer(input_layer, depth, [3, 3], [
                                            1, 1], dilation_rate=self._atrous_rate[1], name=name + 'conv3x3_2')
        _conv3x3_2 = self._get_relu_activation(
            _conv3x3_2, name=name + 'conv3x3_2_relu')

        _conv3x3_3 = self._get_conv2d_layer(input_layer, depth, [3, 3], [
                                            1, 1], dilation_rate=self._atrous_rate[2], name=name + 'conv3x3_3')
        _conv3x3_3 = self._get_relu_activation(
            _conv3x3_3, name=name + 'conv3x3_3_relu')

        _avg_pool = tf.reduce_mean(
            input_layer, self._avg_pool_axes, name=name + 'avg_pool', keepdims=True)
        ...
        _concat_features = tf.concat([_conv1x1, _conv3x3_1, _conv3x3_2, _conv3x3_3,
                                      _img_lvl_upsample], axis=self._feature_map_axis, name=name + 'concat')
        _conv1x1_aspp_out = self._get_conv2d_layer(_concat_features, depth, [1, 1], [
                                                   1, 1], name=name + 'out_conv1x1')
    ...
    def _get_conv2d_layer(self, input_layer, num_filters, kernel_size, strides, dilation_rate=1, use_bias=True, name='conv'):
        return tf.layers.conv2d(inputs=input_layer, filters=num_filters, kernel_size=kernel_size, strides=strides, use_bias=use_bias, padding=self._padding, data_format=self._data_format, kernel_initializer=self._initializer, dilation_rate=dilation_rate, name=name)
    ...
    def _get_upsample_layer(self, input_layer, target_size, name='upsample'):
        ...
        _upsampled = tf.image.resize_bilinear(
            input_layer, target_size, name=name)
    ...
    # convolution layer     #
    #-----------------------#
    def _res_conv_layer(self, input_layer, name, strides=[1, 1, 1, 1], dilations=[1, 1, 1, 1]):
        ...
        x = tf.nn.conv2d(input_layer, filter=W, strides=strides, padding=self._padding,
                         data_format=self._encoder_data_format, dilations=dilations, name=name + '_conv')
        x = tf.nn.bias_add(
    ...
    # convolution block     #
    #-----------------------#
    def _res_conv_block(self, input_layer, stage, strides, dilations=[1, 1, 1, 1]):
        x = self._res_conv_layer(
            input_layer, name='res' + stage + '_branch2a', strides=strides, dilations=dilations)
        x = self._get_batchnorm_layer(x, name='bn' + stage + '_branch2a')
        x = tf.nn.relu(x, name='relu' + stage + '_branch2a')

        x = self._res_conv_layer(
            x, name='res' + stage + '_branch2b', dilations=dilations)
        x = self._get_batchnorm_layer(x, name='bn' + stage + '_branch2b')
        x = tf.nn.relu(x, name='relu' + stage + '_branch2b')

        x = self._res_conv_layer(
            x, name='res' + stage + '_branch2c', dilations=dilations)
        x = self._get_batchnorm_layer(x, name='bn' + stage + '_branch2c')

        shortcut = self._res_conv_layer(
            input_layer, name='res' + stage + '_branch1', strides=strides, dilations=dilations)
    ...
    # identity block        #
    #-----------------------#
    def _res_identity_block(self, input_layer, stage, dilations=[1, 1, 1, 1]):
        x = self._res_conv_layer(
            input_layer, name='res' + stage + '_branch2a', dilations=dilations)
        x = self._get_batchnorm_layer(x, name='bn' + stage + '_branch2a')
        x = tf.nn.relu(x, name='relu' + stage + '_branch2a')
