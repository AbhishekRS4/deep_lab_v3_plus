...
def compute_metrics(groundtruth, predicted_logits, axis=1, num_classes=15):
    predicted_labels = tf.argmax(tf.nn.softmax(
        predicted_logits, axis=axis), axis=axis)
...
def batch_train(FLAGS):
    ...
    net_arch = deep_lab_v3_plus_model.DeepLab3Plus(
