...
class DeepLab3Plus:
    ...
    def deeplabv3_plus(self):
        ...
        self.low_level_features = self._get_relu_activation(self.low_level_features, name="low_level_relu")
        ...
        self.decoder_conv2 = self._get_conv2d_layer(self.decoder_conv1, 256, [3, 3], [1, 1], name="decoder_conv2")
        ...
        self.logits = self._get_upsample_layer(self.decoder_conv3, self._input_size, name="logits")
    ...
    def _atrous_spatial_pyramid_pool_block(self, input_layer, depth=256, name="aspp_"):
        ...
        _conv3x3_1 = self._get_conv2d_layer(
            input_layer, depth, [3, 3], [1, 1], dilation_rate=self._atrous_rate[0], name=name + "conv3x3_1")
        _conv3x3_1 = self._get_relu_activation(_conv3x3_1, name=name + "conv3x3_1_relu")

        _conv3x3_2 = self._get_conv2d_layer(
            input_layer, depth, [3, 3], [1, 1], dilation_rate=self._atrous_rate[1], name=name + "conv3x3_2")
        _conv3x3_2 = self._get_relu_activation(_conv3x3_2, name=name + "conv3x3_2_relu")

        _conv3x3_3 = self._get_conv2d_layer(
            input_layer, depth, [3, 3], [1, 1], dilation_rate=self._atrous_rate[2], name=name + "conv3x3_3")
        ...
        _img_lvl_conv1x1 = self._get_relu_activation(_img_lvl_conv1x1, name=name + "img_lvl_conv1x1_relu")

        _img_lvl_upsample = self._get_upsample_layer(_img_lvl_conv1x1, _inputs_size, name=name + "upsample")
        ...
        _conv1x1_aspp_out = self._get_conv2d_layer(
            _concat_features, depth, [1, 1], [1, 1], name=name + "out_conv1x1")
        _conv1x1_aspp_out = self._get_relu_activation(_conv1x1_aspp_out, name=name + "out_conv1x1_relu")
        _conv1x1_aspp_out = self._get_dropout_layer(_conv1x1_aspp_out, rate=0.1, name=name + "dropout")
    ...
    def _get_conv2d_layer(self, input_layer, num_filters, kernel_size, strides, dilation_rate=1, use_bias=True, name="conv"):
        conv_2d_layer = tf.layers.conv2d(
            ...
            padding=self._padding, data_format=self._data_format, kernel_initializer=self._initializer, dilation_rate=dilation_rate, name=name
        )
    ...
    def _get_dropout_layer(self, input_layer, rate=0.1, name="dropout"):
        dropout_layer = tf.layers.dropout(
            inputs=input_layer, rate=rate, training=self._is_training, name=name
        )
        return dropout_layer

    # return batch normalization layer
    def _get_batchnorm_layer(self, input_layer, name="bn"):
        bn_layer = tf.layers.batch_normalization(input_layer,
            axis=self._feature_map_axis, training=self._is_training, name=name)
        return bn_layer
    ...
    def _res_conv_block(self, input_layer, stage, strides, dilations=[1, 1, 1, 1]):
        ...
        x = self._res_conv_layer(x, name="res" + stage + "_branch2b", dilations=dilations)
        x = self._get_batchnorm_layer(x, name="bn" + stage + "_branch2b")
        x = tf.nn.relu(x, name="relu" + stage + "_branch2b")

        x = self._res_conv_layer(x, name="res" + stage + "_branch2c", dilations=dilations)
    ...
    def _res_identity_block(self, input_layer, stage, dilations=[1, 1, 1, 1]):
        x = self._res_conv_layer(input_layer, name="res" + stage + "_branch2a", dilations=dilations)
        x = self._get_batchnorm_layer(x, name="bn" + stage + "_branch2a")
        x = tf.nn.relu(x, name="relu" + stage + "_branch2a")
