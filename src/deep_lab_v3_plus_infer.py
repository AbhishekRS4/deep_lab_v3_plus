...
from deep_lab_v3_plus_utils import read_config_file, init

os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"
IMAGENET_MEAN = np.array([103.939, 116.779, 123.68]).reshape(1, 3)
param_config_file_name = os.path.join(os.getcwd(), "deep_lab_v3_plus_config.json")
...
def infer(FLAGS):
    ...
    deep_lab_3_plus.resnet50_encoder(image_features_infer)
    deep_lab_3_plus.deeplabv3_plus()
    network_logits = deep_lab_3_plus.logits
    ...
    print(f"Running inference on data : {images_dir_infer}")
    ...
    for img_file in list_images_infer:
        ...
        cv2.imwrite(
            os.path.join(os.getcwd(), model_dir, labels_dir, "label_" + img_file.split("/")[-1]),
            labels_predicted)
